...
from networks.dunet import nlunet
from networks.dinknet import LinkNet34, DinkNet34, DinkNet50, DinkNet101, DinkNet34_less_pool,NLinkNet
...
if __name__ == '__main__':
    ...
    solver = MyFrame(nlunet,dice_bce_loss, 1e-4)
    ...
    for epoch in range(1, total_epoch + 1):
        ...
        mylog.write('--epoch:' + str(epoch) + '  --time:' + str(int(time() - tic)) + '  --train_loss:' + str(
            train_epoch_loss.item()) + ' --val_loss:' + str(val_loss.item()) + ' --val_iou:' + str(
            batch_iou.item()) + '\n')
        print('--epoch:', epoch, '  --time:', int(time() - tic), '  --train_loss:', train_epoch_loss.item(),
              ' --val_loss:', val_loss.item(), ' --val_iou:', batch_iou.item())
